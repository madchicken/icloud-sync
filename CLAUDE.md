...
- 2FA in daemon mode: the daemon writes `~/.icloud_sync_2fa_request` and blocks (10 min) for a code at `~/.icloud_sync_2fa_code`. The app's `TwoFactorWatcher` polls for the marker and prompts; `echo CODE > ~/.icloud_sync_2fa_code` still works by hand.
